...
export class EAPPacketHandler {
    constructor(eapMethods, logger) {
        this.eapMethods = eapMethods;
        this.identities = new NodeCache({ useClones: false, stdTTL: 60 }); // queue data maximum for 60 seconds
        // 	private eapConnectionStates: { [key: string]: { validMethods: IEAPMethod[] } } = {};
        this.eapConnectionStates = new NodeCache({ useClones: false, stdTTL: 3600 }); // max for one hour
    ...
    async handlePacket(packet, handlingType) {
        ...
        // EAP MESSAGE
        let msg = packet.attributes['EAP-Message'];
        ...
        try {
            ...
            const currentState = this.eapConnectionStates.get(stateID);
            switch (code) {
                case 1: // for request
                case 2: // for response
                    switch (type) {
                        case 1: // identifiy
                            this.logger.debug('>>>>>>>>>>>> REQUEST FROM CLIENT: IDENTIFY', stateID, data.toString());
                            if (data) {
                            ...
                            if (currentState.validMethods.length > 0) {
                                this.logger.debug(`using next valid method: EAP Type = ${currentState.validMethods[0].getEAPType()}`);
                        ...
                        case 3: // NAK
                            ...
                            if (data) {
                                ...
                                currentState.validMethods = currentState.validMethods.filter((method) => supportedEAPMethods.includes(method.getEAPType()) // kick it out?
                                );
                                // save
                        ...
                        default: {
                            ...
                            if (eapMethod) {
                                return eapMethod.handleMessage(identifier, stateID, msg, packet, this.identities.get(stateID));
                            }
                            // we do not support this auth type, ask for something we support
                            const serverSupportedMethods = currentState.validMethods.map((method) => method.getEAPType());
            ...
            this.logger.error('decoding of (generic) EAP package failed', msg, err, this.eapConnectionStates.get(stateID));
            return {};
        }
    }
}
//# sourceMappingURL=EAPPacketHandler.js.map