...
import { encodeTunnelPW, startTLSServer } from '../../../../tls/crypt.js';
...
export class EAPTTLS {
    constructor(authentication, tlsOptions, innerTunnel, logger, secret, vlan) {
        this.authentication = authentication;
        this.tlsOptions = tlsOptions;
        this.innerTunnel = innerTunnel;
        this.secret = secret;
        this.vlan = vlan;
        this.lastProcessedIdentifier = new NodeCache({ useClones: false, stdTTL: 60 });
        // { [key: string]: Buffer } = {};
        this.queueData = new NodeCache({ useClones: false, stdTTL: 60 }); // queue data maximum for 60 seconds
        this.openTLSSockets = new NodeCache({ useClones: false, stdTTL: 3600 }); // keep sockets for about one hour
        this.logger = logger.context('EAPTTLS');
    }
    getEAPType() {
        return EAPMessageType.TTLS;
    }
    identify(identifier, stateID) {
        /**
         *  Flags
         *         0   1   2   3   4   5   6   7
         *       +---+---+---+---+---+---+---+---+
    ...
    buildEAPTTLS(identifier, msgType = EAPMessageType.TTLS, msgFlags = 0x00, stateID, data, newResponse = true, maxSize = (MAX_RADIUS_ATTRIBUTE_SIZE - 5) * 4) {
    ...
    buildEAPTTLSResponse(identifier, msgType = EAPMessageType.TTLS, msgFlags = 0x00, stateID, data, newResponse = true) {
        const resBuffer = this.buildEAPTTLS(identifier, msgType, msgFlags, stateID, data, newResponse);
        const attributes = [['State', Buffer.from(stateID)]];
        let sentDataSize = 0;
        do {
    ...
    authResponse(identifier, success, socket, packet) {
    ...
    async handleMessage(identifier, stateID, msg, packet) {
        if (identifier === this.lastProcessedIdentifier.get(stateID)) {
            this.logger.debug(`ignoring message ${identifier}, because it's processing already... ${stateID}`);
            return {};
        }
        ...
        try {
            ...
            let connection = this.openTLSSockets.get(stateID);
            if (!connection) {
            ...
            const incomingMessageHandler = async (incomingData) => {
                const ret = {};
                ret.attributes = {};
                ret.raw_attributes = [];
                ...
                const attributes = {};
                AVPs.forEach((avp) => {
                    attributes[radius.attr_id_to_name(avp.type)] = avp.data;
                });
                ...
                if (result.code === PacketResponseCode.AccessReject ||
                    result.code === PacketResponseCode.AccessAccept) {
                    sendResponsePromise.resolve(this.authResponse(identifier, result.code === PacketResponseCode.AccessAccept, connection.tls, {
                        ...packet,
                        attributes: {
                            ...packet.attributes,
                ...
                connection.events.emit('encrypt', this.buildAVP(radius.attr_name_to_id('EAP-Message'), eapMessage[1]));
            };
            ...
            const responseHandler = (encryptedResponseData) => {
                ...
                while (tlsbuf.length > 5) {
                    ...
                    this.logger.debug(`TLS contentType = ${tlsContentType} version = 0x${tlsVersion.toString(16)} tlsLength = ${tlsLength}, tlsBufLength = ${tlsbuf.length}`);
                    if (tlsbuf.length < tlsLength + 5) {
                        this.logger.debug(`Not enough data length! tlsbuf.length < ${tlsbuf.length} < ${tlsLength + 5}`);
                        break;
                    }
                ...
                // send back...
                sendResponsePromise.resolve(this.buildEAPTTLSResponse(identifier, 21, 0x00, stateID, sendChunk));
            };
    ...
    transformAttributesArrayToMap(attributes) {
        const result = {};
    ...
    buildAVP(code, data, flags = { Mandatory: true }) {
        /**
         * 4.1.  AVP Header
...
//# sourceMappingURL=EAP-TTLS.js.map