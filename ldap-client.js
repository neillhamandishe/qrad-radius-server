import  ldap, { createClient } from "ldapjs";
// import {promisify} from "node:util";
// import * as BPromise from "bluebird";

import * as pkg from "@google-cloud/promisify";
const {promisify, promisifyAll} = pkg;
...
const asyncClient = promisifyAll(client);
const bindLDAP = async (username, password)=>{
	const dn = `ad\\${username.value}`;
	let authenticated = false;
	await asyncClient.bind(username, password.value.toString())
	.then(()=>{
